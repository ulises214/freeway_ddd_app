...
final _phoneRegExp = RegExp(r'^\(?([0-9]{3})\)?([ .-]?)([0-9]{3})\2([0-9]{4})$');
...
abstract class FormValidators {
  ...
  static String? _isValidEmail(String email) => GetUtils.isEmail(email) ? null : 'Email no válido';

  static String? _isValidPassword(String password) =>
      password.length < 8 ? 'La contraseña debe contener al menos 8 caracteres' : null;
  ...
  static String? isEmailAnOptional(String? email) => isOptional(email, _isValidEmail);

  /// The given [email] is required and is a valid email addres
  static String? isEmailAndRequired(String? email) => isRequired(email, _isValidEmail);
}
